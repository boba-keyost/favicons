    ...
    HTML: {
      parse(html) {
        return new Promise(function (resolve) {
          ...
          var tag = $("*", "head").first();
          var isLink = tag.is("link");
          var attribute = isLink ? "href" : "content";
          ...
          if (isLink && tag.attr("rel") === "manifest" && options.loadManifestWithCredentials) {
      ...
      render(sourceset, properties, offset) {
        ...
        if (svgSource) {
          ...
          promise = Jimp.read(nearestIcon.file).then(function (image) {
            return image.contain(width, height, Jimp.HORIZONTAL_ALIGN_CENTER | Jimp.VERTICAL_ALIGN_MIDDLE, options.pixel_art ? Jimp.RESIZE_NEAREST_NEIGHBOR : null);
          });
        }
